...
def func_main(df, meta_op):
    ...
    model_param_before = np.array(model_before.feature_importances_)
    model_param_before = model_param_before.reshape(1, df.shape[1]-1)
    with open('Coef1.csv', 'w') as csvfile:
        writer = cv.writer(csvfile)
        writer.writerow(df.columns.values)
        writer.writerows(model_param_before)
    model_param_after = np.array(model_after.feature_importances_)
    model_param_after = model_param_after.reshape(1, dfMod.shape[1]-1)
